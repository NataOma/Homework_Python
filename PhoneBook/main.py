...
def write_line_tofile(filename): # функция копирования данных из одного файла в другой
    row_number = int(input("Введите номер строки: "))  # номер строки в целочисленном формате
    res = read_file(filename)  # читаем файл функцией read_file(), получаем список словарей
    res2 = [] # создаем пустой список
    res2.append(res[row_number - 1]) # сохраняем строку в новый список
    standard_write('phone2.csv', res2)  # записываем строку в файл при помощи вызова функции standard_write()
...
def main():  # основной модуль с которого начинает работу программа
    while True:  # бесконечный цикл, в котором ожидаем реакции с консоли
        ...
        print(' "d"-удаление из csv; "c"-изменить строку в csv; "a"- копировать строку в другой файл')
        ...
        if command == "q":
            ...
            if not exists('phone2.csv'):  # если файл с указанным именем не существует,
                create_file('phone2.csv') # то создаем его, вызвав функцию create_file()
                continue
            write_line_tofile(filename)
